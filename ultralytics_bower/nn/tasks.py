...
from ultralytics_bower.nn.modules import (
    AIFI,
    C1,
    C2,
    C3,
...
class RTDETRDetectionModel(DetectionModel):
    ...
    def init_criterion(self):
        ...
        from ultralytics_bower.models.utils.loss import RTDETRDetectionLoss
...
def torch_safe_load(weight):
    ...
    """
    from ultralytics_bower.utils.downloads import attempt_download_asset
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        elif m in {Detect, WorldDetect, Segment, Pose, OBB, ImagePoolingAttn, DetectMultiHead}:
...
def guess_model_task(model):
    ...
    def cfg2task(cfg):
        ...
        if m in {"classify", "classifier", "cls", "fc"}:
            return "classify"
        if m in {"detect", "detectmultihead"}:
            return "detect"
        if m == "segment":
            return "segment"
        if m == "pose":
    ...
    if isinstance(model, nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, Segment):
                ...
                return "obb"
            elif isinstance(m, (Detect, WorldDetect, DetectMultiHead)):
                return "detect"
